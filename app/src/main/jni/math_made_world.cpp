...
struct _random {
    ...
    double nextDouble(double l = 0, double r = 1) {
...
struct _miller {
    ...
    inline ll mul(bigint x, ll y, ll mod) {
        auto hi = bigint(x) * bigint(y) % bigint(mod);
...
Java_happy_mjstudio_harlequin_util_NativeLib_prime_1factorize_1zirige_1fast(JNIEnv *env,
    ...
    auto ret = pollard.factorize(n);
    string str;

    for (int i = 0; i < ret.size(); i++) {
        str += to_string(ret[i].first);
        if (ret[i].second > 1) str += "^" + to_string(ret[i].second);
        if (i != ret.size() - 1) str += " * ";
    }
